...
export default function Cloud() {
    ...
    const handleActualNumItems = () => {
        if (actualNumItems >= certificatesInfo.cloud.length) {
            setActualNumItems(6);
            setActualItems(certificatesInfo.cloud.slice(0, 6));
        } else {
            setActualNumItems(actualNumItems + 6);
            setActualItems(certificatesInfo.cloud.slice(0, actualNumItems + 6))
        }
    }
    return (
        <section ref={refAbout} className={`flex flex-col w-1/2 h-auto justify-center items-start text-white pt-40 opacity-0 ${isVisibleAbout ? 'animate-fade-bottom-y-4' : ''}`} style={{animationDelay: ".2"}} id='cloud'>
            <div className='flex flex-row w-full h-auto justify-end items-center px-10 mb-32'>
                <h1 className='font-bold text-4xl w-1/3 flex justify-center items-center'><span className='text-[#2E28A0] mr-5'>02.</span> Cloud</h1>
            ...
            {certificatesInfo.cloud.length > 6 && <div className="w-full h-auto flex flex-row justify-end items-center animate-fade-bottom-y-5 opacity-0 mt-16" style={{ animationDelay: ".8s" }}>
                <span href="#" onClick={handleActualNumItems} className="left-40 bg-[#161616] relative z-30 cursor-pointer flex justify-center items-center w-40 h-12 rounded-lg border-2 border-[#2E28A0] font-bold text-xl text-white transform transition duration-300 hover:-translate-x-1 hover:-translate-y-1">
                    {actualNumItems >= certificatesInfo.cloud.length ? "Show less" : "Show more"}
                </span>
